---
title: "Backfill"
description: "probe backfill — point an agent at a folder of work you have already done."
...
Backfill is the no-connector path: it reads a folder on this machine, works out what is in it, uploads what it finds and describes it. The [wizard](/wizard/import) runs the same thing with a screen and in the background; the CLI runs it synchronously.

Both share the same file-coverage records and delivery receipts, so **completed uploads are reused whichever entry point you use.**
...
See [Existing imports](/wizard/imports) for the states and what each one means.
